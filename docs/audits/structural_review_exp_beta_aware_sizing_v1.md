...
**Status:** RESOLVED (operator decisions accepted 2026-05-31). exp_beta_aware_sizing_v1 CLOSED at T15 via Branch A; B1 (no-notional observation mode) authorized as next active work; B2/C trading paths declined; next strategic direction deferred to post-B1. Triggered by the E4 mechanical halt at T15 (9/15 = 60.0% coint-failure rate), not by the 20-trade gate (superseded by the halt).
...
- [x] **Accept the negative result for this configuration?** → **DECIDED: YES (Branch A accepted).** The convergent stack supports it (H1 settled + every-clearance-within-noise + mean-shift β-independent + eligible stall + E4 halt fired cold + win-within-noise); the pre-committed §5 bar's bottom-line antecedent is met. exp_beta_aware_sizing_v1 closed at T15 with finding: *sizing solved, no robust demonstration of capturable edge above costs at $200 notional on this universe at the current cost-model precision, dominant loss mode entry-unpredictable mean-shift, universe coint-fragile (E4 halt).*
- [x] **Build the no-notional observation mode (B1)?** → **DECIDED: YES (authorized).** B1 is the next active work item — the only instrument that resolves the open temporal-vs-structural fragility question without trading. Read-only on monitoring loop, no PnL → marking-fidelity wall doesn't apply. Tests RISK_OFF vector at N. Scope: separate engineering task; design + implementation + test before deploy. The validator-sidecar pattern (`tools/fidelity_validator/`) is the established precedent for read-only observation instruments.
- [x] **Pursue B2/C as trading experiments now?** → **DECIDED: NO.** B2 (cost-precision via more trades) is foreclosed by the cost-model precision ceiling — more trades within the same ±$0.06 don't isolate the mechanism; B2 should be pursued as a separate cost-instrument upgrade (per-fill attribution), not as a trading experiment. C (universe restriction) rests on N=2 directional hypotheses and would require un-halting a fragile universe to sample — wrong instrument. The decision is locked against drift; reopening either requires new evidence clearing the §5 bar.
- [x] **Next strategic direction** → **DECIDED: DEFER to post-B1.** The three candidate directions (different universe / different hold horizon / different strategy class) each require fragility evidence B1 will produce. Picking now is premature. B1 data + the RISK_OFF vector test characterize which universes are less fragile and discriminate temporal vs structural. Direction picked after B1 lands.
...
*Structural review v1.0 (DRAFT for operator review; §0 synthesis + §5.1 reframe + §5.2 point-1 + §6 stack-line-2 + Branch A foreclosure language + §9 chain-steps 2 and 4 corrected on first-read verification — the strong reframe inverted the cost-adjustment direction (generous vs adverse) and overstated what "0/6 ROBUST-PASS" warrants; the corrected version states the data exactly: every clearance sits within the cost-model's ±$0.06 noise band, including the win, with 0/6 ROBUST-PASS and 2/6 ROBUST-FAIL even under generous as the diagnostic's robust signals. Softer in form, identical in direction; sources the §5 bar's bottom-line antecedent from the diagnostic AS CORROBORATED by the convergent stack, not from the diagnostic standing alone). Triggered by E4 halt at T15 (9/15 = 60.0%). §7 centerpiece: cost_diagnostic_post_T15_halt.md (AMBIGUOUS-at-N; 0/6 ROBUST-PASS; 2/6 ROBUST-FAIL under generous). Pre-committed criteria: structural-review template v1.4/v1.5. Inputs: per-run audit T1–T15 (runs 125–142); CURRENT_STATE; the cost diagnostic; the query-3 redirect terminal finding; the prior exp_coint_stability_v1 review. Verdict structure per operator Q2: findings + decision tree + recommended decision with reasoning chain visible. Recommendation: Branch A (accept negative result for this configuration) + B1 (no-notional observation mode) as the worthwhile follow-on; not B2/C as trading experiments. The call is the operator's; the chain is shown so disagreement can be placed at the right link.*
