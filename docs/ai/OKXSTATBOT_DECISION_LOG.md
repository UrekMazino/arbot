...
Config Trap — limit_order_basis is a misnomer; entries are always market/taker (2026-05-29):
- Type: Documented config trap (Section-8A class: verify behavior against code, not names). Same class as the [0.3,3.0] discovery filter vs [0.20,5.00] sizing-bound finding (2026-05-29).
- Trigger: taker-vs-maker investigation for the exp_beta_aware_sizing_v1 structural review (cost-lever question, Branch 2).
- Claim vs reality: STATBOT_LIMIT_ORDER_BASIS (config_execution_api.py:276) defaults True and was commented "Place entries with limit orders when True." FALSE. Entries are ALWAYS market orders (taker): func_trade_management.py:3469 (long) and :3558 (short) call initialise_order_execution() with no limit_offset → defaults 0.0 → func_execution_calls.py:733 `order_type = "limit" if limit_offset and limit_offset != 0 else "market"` selects "market" → place_market_order (:951, ordType="market"). No postOnly / no TIF flag exists anywhere in the Execution layer, so maker is never enforced even when a limit path is taken.
- What the flag actually gates (entry-LOOP semantics, not order type): capital-injection sizing (func_trade_management.py:2900 — when True, initial injection sized to min(liquidity, capital) vs full capital when False); entry-fill logging (:3642, fires when False); kill-switch timing (:3712, sets kill_switch=1 after both legs fill when False — one-shot market termination). The True path runs a multi-pass/liquidity-sized loop with a 3s "allow limit order to register" wait, but still places market orders. Hybrid state: limit-order-basis loop semantics driving market/taker placement.
- Corroborating design intent: func_calculation.py:152-162 pads entry price by FEE_SLIPPAGE_BUFFER_PCT, commented as accounting for "taker fee (0.05%) + slippage (0.02%)" — the price model assumes taker fills.
- Consequence for the experiment: the maker cost lever (Branch 2b) is REAL and AVAILABLE (entries currently pay taker + half-spread), but a maker conversion is NEW CODE — thread limit_offset/postOnly through initialise_order_execution → place_entry_with_stop — NOT a config flip. Do not mistake STATBOT_LIMIT_ORDER_BASIS=true for "already maker."
- Fix applied: corrected the misleading comment at config_execution_api.py:276 to describe actual behavior. No code behavior changed (mid-experiment, frozen config).
- §7 data-availability note (same investigation): liquidity_checks.csv logs depth (liquidity_long/short_usdt, ratios) but NOT bid-ask spread. The refined residual-vs-effective-half-spread cost diagnostic cannot run off existing telemetry as-is; half-spread must be reconstructed from entry orderbook snapshots (persistence unconfirmed). Feasibility check required before Branch 2 cost diagnostic.

exp_beta_aware_sizing_v1 — STRUCTURAL REVIEW VERDICT (Branch A accepted, 2026-05-31):
- Type: Experiment closure + next-direction decision.
- Trigger: E4 mechanical halt fired at T15 (9/15 = 60.0% coint-failure rate; 5-deep coint-failure run T11–T15 at ~3% probability under 50% base rate). Pre-commit decided cold at T13 (template v1.4 §4) executed without in-the-moment deliberation.
- Inputs: 15 closed trades (T1–T15, runs 125–142); per-run audit; cost diagnostic on 5 $/σ-eligible + T15 paired exhibit; halt-interpretation pre-load (template v1.5 §4); convergent evidence stack.
- Diagnostic verdict: AMBIGUOUS-at-N (three analytical slices agree). At ±$0.06 cost-model precision against ~$0.10 edges, the model is coarser than the signal; the AMBIGUOUS verdict is structural, not a property of N alone.
- Robust diagnostic sub-findings: every cost-clearance in the eligible population sits within the ±$0.06 noise band including the one realized win (T7 +$0.015 point → sign-flips −$0.045 under adverse); 0/6 ROBUST-PASS; 2/6 ROBUST-FAIL even under generous cost (T2 LTC/KSM thin-leg + recon-FAIL; T6 SOL/AVAX liquid pair clean recon) — cost-too-high sub-mode established at N=2 directional.
- Convergent evidence stack: (1) H1 = CLEAN SUCCESS (β-sizing 15/15 mechanically exact across β range [0.378, 1.841]; 5/5 $/σ-eligible sign-positive; aggregate +$0.044/σ; settled finding); (2) every diagnostic clearance within noise; (3) mean-shift β-independent, entry-unpredictable (6/9 clean coint-failures DECOUPLED; demonstrated at β ∈ {0.378, 0.456, 0.476, 0.561, 0.667, 1.495, 1.841}); (4) eligible-population stall (5 eligible in T2–T8, 0 in T9–T15); (5) E4 halt fired on genuine trajectory; (6) T15 anchor exhibit (mechanism worked, edge below cost in a single clean trade).
- Branch A — ACCEPT NEGATIVE RESULT for this configuration: DECIDED. Finding: *sizing solved; no robust demonstration of capturable edge above costs at $200 notional on this universe at the current cost-model precision; dominant loss mode entry-unpredictable mean-shift; universe coint-fragile.* The §5 pre-committed negative-result bar's bottom-line antecedent (no demonstrated capturable edge at current notional and universe) is met from the diagnostic AS CORROBORATED by the convergent stack — not from the diagnostic standing alone. exp_beta_aware_sizing_v1 CLOSED at T15.
- B1 — Build no-notional observation mode: AUTHORIZED as next active work. Only instrument that resolves the open TEMPORAL vs STRUCTURAL fragility discriminator (eligible-return rate post-halt) without trading; tests the RISK_OFF-entry vector at N. Read-only on monitoring loop, no PnL → marking-fidelity wall (which killed query-3) does not apply. Pattern precedent: `tools/fidelity_validator/` (read-only sidecar). NOT YET BUILT — separate scoped engineering task.
- B2 / C — Trading experiments: DECLINED. B2 (cost-precision via more trades) is foreclosed by the cost-model precision ceiling; the path forward on cost is an instrument upgrade (per-fill attribution), separately, not a trading experiment. C (universe restriction on N=2 directional hypotheses) would require un-halting a fragile universe to sample; wrong instrument — B1 tests restriction hypotheses without trading. Locked against drift; reopening either requires new evidence clearing the §5 bar.
- Next strategic direction: DEFERRED until B1 data exists. Three candidate directions (different universe / different hold horizon / different strategy class) each require fragility evidence B1 produces.
- Findings locked from this experiment (carry forward): (a) H1 β-aware sizing aligns signal and position (HIGH confidence, settled); (b) mean-shift is the dominant β-independent entry-unpredictable loss mechanism in this universe (HIGH confidence); (c) cost-model precision (±$0.06 vs ~$0.10 edges) is a structural constraint on resolving edge-vs-cost at this notional; (d) consecutive-loss circuit breaker is structurally inert under max_session_trades=1 (latent scale-up gap, recorded for future).
- Review document: docs/audits/structural_review_exp_beta_aware_sizing_v1.md. §7 centerpiece: docs/audits/cost_diagnostic_post_T15_halt.md. Pre-committed criteria source: structural-review template v1.4/v1.5.
- Discipline note: the §5.1 reframe was checked on first-read and corrected — the strong claim "even under generous cost, nothing clears" inverted the cost-adjustment direction; the corrected reading ("every clearance within the cost-model's ±$0.06 noise band") is softer in form but supports the same conclusion via the convergent stack rather than via the diagnostic standing alone. The correction is in the review's footer and audit trail.
