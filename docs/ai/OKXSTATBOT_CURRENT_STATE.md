...
- trades_since_experiment_start: 13 (+T13 BTC/DOGE r140)
- $/σ sign-flip rate: 0/5 = 0% (eligible = T2, T5, T6, T7, T8; all positive. T1, T3, T4, T9, T11, T12, T13 excluded as coint-failures; T10 excluded as adverse-normal MFE<0)
- aggregate $/σ (pooled): +$0.044/σ unchanged (H1 rock-solid 5/5)
- coint-failure count (window): **7/13 = 53.8%** (T1, T3, T4, T9, T11, T12, T13) — trajectory 75→60→50→37.5→44.4→50→**53.8** (last 3 trades all coint-failures — directional uptick reversed the favorable trend; into upper part of 45–60% band)
- **E4 EVALUABLE: 53.8% in upper part of 45–60% REVIEW band, NOT HALT** per calibration. Trajectory has reversed favorable→elevated over last 4 trades. For halt requires next ~2-3 trades all coint-failures to push >60%. Continue flag for structural review; pre-commit held.
- adverse-normal-exits bucket: 1 (T10)
- **beta_range_observed: [0.378, 1.841]** (T13 β=1.8414 — HIGHEST β observed, supra-unity); β<1.0: 10/13; materially non-unity: 10/13; fallback: 0; β-sizing flawless 13/13
- cumulative PnL (experiment window): **-$4.016** (T13 -0.331; **4 LOSSES IN A ROW** T10-T13 = -$2.144). win rate 1/13 = 7.7%
...
- audit: docs/audits/exp_beta_aware_sizing_v1_per_run_audit.md (runs 125-140, T1-T13)
