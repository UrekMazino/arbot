...
def test_pair_metric_cache_persists_when_output_write_is_disabled(monkeypatch):
    monkeypatch.setattr(fc, "time_frame", "1m")
    monkeypatch.setattr(fc, "_load_restricted_tickers", lambda: set())
    monkeypatch.setenv("STATBOT_STRATEGY_PAIR_METRIC_CACHE", "1")
    monkeypatch.setenv("STATBOT_STRATEGY_PAIR_METRIC_CACHE_PERSIST", "1")
    ...
    assert calls["count"] == 1
    assert first_summary["pair_metric_cache"]["persisted"] is True
    assert first_summary["pair_metric_cache"]["writes"] == 1

    def fail_cointegration(*_args, **_kwargs):
        raise AssertionError("write_output=False should still reuse persisted metric cache")
