...
def build_exit_decision_trace_payload(
    *,
    ...
    if not full_tp_zone_hit:
        why_full_tp_not_selected = "outside_full_tp_zone"
    elif full_tp_selected:
        why_full_tp_not_selected = "selected"
    elif not full_tp_guard_passed:
        why_full_tp_not_selected = "net_profit_guard_blocked"
    elif "trade_manager_take_profit" in {name.strip() for name in blocked_candidate_names.split(",") if name.strip()}:
        why_full_tp_not_selected = "orchestrator_net_profit_guard_blocked"
    elif selected_candidate:
        why_full_tp_not_selected = f"preempted_by_{selected_candidate}"
    elif selected_exit_action and selected_exit_action.lower() != "hold":
        why_full_tp_not_selected = f"preempted_by_{selected_exit_action}"
    else:
