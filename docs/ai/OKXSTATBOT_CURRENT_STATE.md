...
- trades_since_experiment_start: 6 (T1 JUP/YGG run_125, T2 LTC/KSM run_126, T3 BNB/LINK run_129, T4 DOGE/AAVE run_130, T5 AVAX/DOT run_131, T6 SOL/AVAX run_132)
- $/σ sign-flip rate: 0/3 = 0% (computable population = T2, T5, T6; all positive. T1, T3, T4 excluded as coint-failures under Rule v1.2)
- aggregate $/σ (pooled): +$0.029/σ (positive; T2 +$0.064, T5 +$0.017, T6 +$0.020) — H1 holding strong, β-sizing aligns signal/position 3/3
- coint-failure count (window): 3/6 = 50% (T1, T3, T4) — trending DOWN (75%→60%→50%)
- E4 WATCH (not firing): coint-failure 50% now mid-band (45–60% review). E4 needs ≥10 closed (currently 6). Eligible population growing (3 in last 2 trades) — dilution looked worse at T4 than now.
- beta_range_observed: [0.378, 1.495] (T6 β=0.911 = first near-unity); β<1.0: 4/6; materially non-unity: 5/6; fallback: 0; β-sizing flawless 6/6 (exact to the cent)
- cumulative PnL (experiment window): -$1.746 (T1: -$0.962, T2: -$0.105, T3: -$0.267, T4: -$0.253, T5: -$0.030, T6: -$0.1285)
- edge_clears_costs: 0/3. KEY (PnL-vs-z diagnostic): pnl_at_mean > cost is NO on all 3 eligible (T2 +$0.052, T5 +$0.052, T6 +$0.026 — all far below costs; T6 in-zone max PnL just +$0.007). Thesis-exit edge has never cleared costs. Apparent edge (MFE) lives at overshoot (anti-thesis). → Branch 2, sub-lever undetermined (cost-too-high vs edge-too-thin); §7 cost diagnostic discriminates. Exit-redesign/Item 14 NOT indicated (no EXIT-TOO-LATE). Profit-lock fired 2/2 below cost-clearance.
- action_threshold: 20 closed trades before structural review
- trades_remaining: 14 (to 20 total); ≥5 more $/σ-eligible needed (3 eligible after 6 trades)
...
- audit: docs/audits/exp_beta_aware_sizing_v1_per_run_audit.md (runs 125-132, T1-T6)
...
- run_132: T6 SOL/AVAX, β=0.911 (first near-unity). gross=$200 to the cent (6/6 exact). 3rd $/σ-ELIGIBLE (normal/profit_lock, MFE +$0.179>0, Δz=3.26). $/σ=+$0.020 POSITIVE (sign-flip 0/3). Gross +$0.065 (worked) but net -$0.1285 (costs $0.194). Near-replica of T5: pnl_at_mean +$0.026, in-zone max PnL +$0.0067 — profit lived at z=+2.25/+3.03 overshoot (anti-thesis). 2nd profit-lock activation, again below cost-clearance. Coint held. Ended via max_session_trades=1.
