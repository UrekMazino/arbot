...
**Halt-interpretation pre-load — TEMPORAL vs STRUCTURAL fragility (2026-05-30, post T14 — same anti-rationalization discipline applied one level up, written cold before the halt may fire).** If the E4 halt fires at T15, the structural-review question is **not** "did E4 fire" — that's just the mechanical trigger. The substantive question is **which of two readings the halt represents**, because they lead to different next-experiment paths and the wrong reading is easy to motivate after the fact. Pre-loading the two readings and the discriminating evidence now, before any halt, removes that motivated-reasoning risk:

> - **Reading 1 — TEMPORAL fragility (universe entered a higher-coint-failure regime around T9).** The trajectory (37.5→44.4→50→53.8→57.1, T9 as the inflection) looks like a regime change, not just small-window variance. The eligible cluster T2/T5/T6/T7/T8 came before T9; the failure cluster T9–T14 came after. Under this reading the halt is saying: *"this market period is too coint-fragile to fairly test sizing."* Next-experiment direction: temporal — wait for the regime to pass, OR detect-and-gate the fragile regime at entry (which connects naturally to the RISK_OFF vector — 2/2 RISK_OFF entries → coint-failure; if high-coint-failure periods are flaggable, that's the surviving lever).
> - **Reading 2 — STRUCTURAL fragility (universe was always this fragile; the early stretch was lucky variance).** The five eligible trades all clustered early; the entire recent run produces zero eligibles. Under this reading the halt is saying: *"this universe does not hold β-fit relationships through a 60-min hold often enough to trade, period."* Next-experiment direction: structural — the strategy class, the universe, or the hold horizon is wrong for this market, not the timing. Stop-and-restart on a different universe (liquidity-floored, lower-fragility) or a different hold horizon (much shorter, exit-on-divergence-not-on-coint-watch).
> - **The discriminator (decided in advance): eligible-return rate after any halt.** If T15+ collection (assuming the halt doesn't actually stop new trades, OR a deliberate post-halt scoping period) produces eligible trades at a rate comparable to T2–T8 (≈ 5 in 8 = 63%), Reading 1 is supported — the early window was not anomalous and the recent absence was a fragile-regime episode. If T15+ continues to produce coint-failures at the recent rate (≥ 75% over the next 6+ trades, with eligibles staying ≤ 1) — Reading 2 is supported — the early cluster was the lucky window. The structural review should explicitly cite *eligible-return rate post-halt* as the deciding evidence, not the coint-failure rate itself (which is already what triggered the halt and is not new information).
> - **Standing read at the moment of the pre-load (post T14, before T15):** the eligible stall *leans toward Reading 2*. Five eligibles all came in T2–T8; six consecutive trades T9–T14 produced zero. If this were a transient fragile regime, you'd expect at least one eligible to break through. Pre-loading this read as the *prior* — not a finding — so the structural review knows where it starts and what evidence would move it (an eligible-return after halt would move it toward Reading 1; sustained absence would confirm Reading 2). The lean is directional, not a verdict; T15 alone resolves the halt-or-continue, not the reading.
> - **Why pre-load now.** Once a halt fires (or doesn't), the in-the-moment narrative pressure will be strong: a halt with "this is a fragile regime, just wait it out" frames it temporally and reopens the door to a delay-then-resume that doesn't really learn anything; a halt with "this universe is structurally broken" frames it structurally and forces the universe/horizon question. Both narratives will feel coherent at the time. Writing the discriminator (eligible-return rate, not coint-failure rate) into the template now means the post-halt review starts from the right *question* — which reading does the post-halt evidence support — rather than from the wrong one ("the halt fired, what do we do"). Same discipline as the E4 mechanical halt: name the branches cold, decide the deciding evidence cold, let the data do the work.
...
**Consecutive-loss circuit-breaker inertness under one-trade-per-session — latent gap, surfaced post-T14 (2026-05-30).** The configured circuit breaker (consecutive-loss limit = 3, mode = session) is **structurally inert** under the current `max_session_trades = 1` setting: every trade is its own session, so the consecutive-loss counter resets each run and cannot accumulate across the experiment window. Observable consequence: T10–T14 produced **5 losses in a row, −$2.572**, and the breaker did not — and *could not by construction* — fire. Working as configured, not a bug; flagged here only because the latent gap matters under scale-up. **For the current experiment this is fine** — E4 is the experiment-level stop and is doing the job the breaker is structurally unable to do at one trade per session; the breaker is the wrong instrument for a research-instrument's single-trade sessions, and changing it mid-window would also be a frozen-variable violation. **Where it matters:** any future move toward multi-trade sessions or live scale-up needs to revisit the breaker's effective semantics. As configured today, a 5-loss / −$2.6 streak (or worse) would pass through unchecked because the session boundary resets the counter. The breaker may need to be re-scoped to *experiment-window* or *rolling-N* mode, *or* the experiment-level analogue of E4 needs to be the named live-safety instrument and the consecutive-loss breaker explicitly retired. Either is defensible; the choice belongs to the scale-up conversation, not this review. Logged here so the scale-up conversation does not start with a false belief that the breaker would have caught this kind of streak — it would not have.
...
*Template version: exp_beta_aware_sizing_v1 structural-review template v1.5 (DRAFT, pre-trigger; §4 gate locked + E4 added; §5 negative-bar approved verbatim; §6/§7 updated post Query 1+2; §6 first SUPERSEDED post T7/T8 — Item 14/Branch 1 REOPENED for edge-bearing subset, fork SUBSET-VIABLE-leading/subset-uncharacterized; v1.4 changes (2026-05-30 post T13): (a) §4 — E4 trajectory pre-commit added for T14–T15 (anti-rationalization lock: both coint → halt mechanically, otherwise variance, continue); (b) §6 — MEAN-SHIFT PROMOTED from "leading surviving hypothesis" to FINDING after T13 β=1.8414 closed the supra-unity case, with the lever-set consequences (entry-time coint metrics definitively ruled out, no future revival without contrary evidence; surviving levers post-entry + structural); **v1.5 changes (2026-05-30 post T14): (a) §4 — HALT-INTERPRETATION PRE-LOAD added (TEMPORAL vs STRUCTURAL fragility, with eligible-return rate post-halt as the decided-in-advance discriminator; standing read leans Reading 2 / structural per T9–T14 eligible stall — directional prior, not verdict; same anti-rationalization discipline as the E4 mechanical halt, one level up); (b) §10 — CONSECUTIVE-LOSS CIRCUIT-BREAKER INERTNESS noted as a latent gap under one-trade-per-session: T10–T14 5-loss / −$2.572 streak did not fire and could not by construction; fine for the research instrument with E4 as the experiment-level stop, but must be re-scoped before any scale-up / multi-trade-session move; logged so scale-up conversation does not start with a false belief**; query-3 redirect to real-trade analysis recorded separately in `docs/prompts/query3_output_spec_universe_scale_joint_distribution_v1.md` v1.3. Prior empirical-update blocks preserved as superseded for the evolution record. Committed criteria (§4 gate, §5 bar, branch definitions) unchanged across all updates.*
