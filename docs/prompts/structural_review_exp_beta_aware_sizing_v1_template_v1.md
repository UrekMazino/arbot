...
> **EMPIRICAL UPDATE (2026-05-29 — SUPERSEDING — post T7 (SOL/CRV) + T8 (BCH/ETC); N=5 eligible — DIRECTIONAL, branches below remain the pre-committed structure):**
...
*Template version: exp_beta_aware_sizing_v1 structural-review template v1.3 (DRAFT, pre-trigger; §4 gate locked + E4 added; §5 negative-bar approved verbatim; §6/§7 updated post Query 1+2; §6 SUPERSEDED post T7/T8 — Item 14/Branch 1 REOPENED for the edge-bearing subset (T8 confirmed EXIT-TOO-LATE), fork status now SUBSET-VIABLE-leading/subset-uncharacterized, E4 deflating at 37.5% below baseline, query-3 hold now soft with §5 fidelity-gate pre-staging recommended; all N=5-eligible directional). Prior post-Query-1+2 block preserved as superseded for the evolution record.*
