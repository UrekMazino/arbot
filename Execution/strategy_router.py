...
class StrategyRouter:
    ...
    def __init__(self, state_store=None, config=None):
        ...
        self.config = {
            ...
            "mr_entry_z": max(_env_float("STATBOT_STRATEGY_MR_ENTRY_Z", _env_float("STATBOT_ENTRY_Z", 2.0)), 0.1),
            "mr_entry_z_max": max(
                _env_float("STATBOT_STRATEGY_MR_ENTRY_Z_MAX", _env_float("STATBOT_ENTRY_Z_MAX", 3.0)),
                0.1,
            ),
            "mr_min_persist": max(
                _env_int("STATBOT_STRATEGY_MR_MIN_PERSIST", _env_int("STATBOT_MIN_PERSIST_BARS", 4)),
                1,
            ),
            "mr_size_mult": max(_env_float("STATBOT_STRATEGY_MR_SIZE_MULT", 1.0), 0.0),
            "mr_min_liq_ratio": max(_env_float("STATBOT_STRATEGY_MR_MIN_LIQ_RATIO", 1.5), 0.0),
