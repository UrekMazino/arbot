...
**Status:** RESOLVED-with-AMENDMENT (operator decisions accepted 2026-05-31; AMENDED same day post-B1-v1 baseline analysis). exp_beta_aware_sizing_v1 CLOSED at T15 (E4 halt); B1 v1 built and ran; **Branch A acceptance now PENDING basis-mismatch diagnostic** (§9.5) because B1 v1 revealed the selector and monitor use different cointegration tests — `health=valid` never observed in 732 samples — and some fraction of the coint-failure count that triggered the halt may be procedural artifact, not universe-fragility. The diagnostic's three verdicts are pre-committed cold in §9.5. B2/C trading paths still declined; next strategic direction deferred until §9.5 diagnostic + B1 v1.1 (cross-run per-pair) land. Triggered by the E4 mechanical halt at T15 (9/15 = 60.0% coint-failure rate).
...
- [~] **Accept the negative result for this configuration?** → **AMENDED: PENDING basis-mismatch diagnostic (§9.5).** Branch A is the lean from the convergent stack and remains the most-likely outcome — but B1 v1's `health=valid never observed in 732 samples` finding (see §4.3) revealed that the bot's selector (orderbook-mid basis) and its post-entry monitor (kline-only basis) measure different things; some fraction of the coint-failure count that triggered the E4 halt may be procedural artifact from basis-mismatch rather than universe-fragility. The dollar-DECOUPLED cases (6/9) are robust to this concern; the TRACKED-THEN-BROKE cases (3/9, including T15) are not. The §5 bar's bottom-line antecedent ("no capturable edge at current notional and universe") implicitly assumes the universe was tested with consistent instruments — which B1 just put in doubt. **Acceptance gated on §9.5 diagnostic clearing; if BASIS-AGREEMENT, Branch A firms cleanly; if BASIS-DISAGREEMENT-SUBSTANTIAL, the negative result is premature and a basis-aligned retest replaces it; if AMBIGUOUS-INSUFFICIENT-PAIRED-DATA, the convergent stack continues to lean Branch A with the open question recorded.** Decision held in this state until diagnostic lands.
...
- [ ] **Run basis-mismatch diagnostic (NEW, §9.5)?** → **RECOMMEND YES.** Read-only analysis on existing logs; pre-committed verdicts below. Cheap; resolves the Branch A precondition. The right next piece of work before any consequential next-direction decision. Not yet started.
- [ ] **Run B1 v1.1 (cross-run per-pair aggregation)?** → **RECOMMEND YES.** Tests whether RISK_OFF elevation is regime-causal or pair-selection-driven (which determines whether the lever is regime-gating or universe-restriction). Data exists in B1 v1 sample CSVs; one aggregation script. Pairs naturally with §9.5.
- [~] **Next strategic direction** → **DEFERRED until §9.5 diagnostic + v1.1 land.** Per the amendment above, the basis-mismatch diagnostic determines whether Branch A is correct or whether a basis-aligned retest replaces it; that resolution gates the direction choice.
...
> **BASIS-AGREEMENT** — the selector and monitor verdicts substantially agree where they can be compared (e.g., at pair-switch moments, post-switch monitor verdicts track what entry-discovery would say on the same pair if re-tested under the orderbook-mid basis; or, when both tests fire on the same pair within a short window, they reach consistent verdicts). The `health=valid never observed in 732 samples` finding is then a property of monitor-threshold strictness, not of basis disagreement on the underlying truth. **Branch A FIRMS cleanly** — the universe-fragility component of the negative result is robust; mean-shift is economic; the §5 bar's antecedent is met without the basis caveat. The 3/9 TRACKED-THEN-BROKE cases were genuine relationship breakdown, not procedural artifact. Operator-decision row for Branch A flips from `[~] PENDING` to `[x] DECIDED YES`.

> **BASIS-DISAGREEMENT-SUBSTANTIAL** — the two bases meaningfully disagree on a non-trivial fraction of cases. A meaningful fraction of monitor-`broken`/`watch` events would still be `valid` under orderbook-mid; some fraction of the TRACKED-THEN-BROKE coint-failures were procedurally triggered by basis-mismatch rather than by relationship breakdown. The 6/9 DECOUPLED cases (real dollar decoupling) remain real — that evidence is basis-independent — but the universe-fragility magnitude is reduced and the E4 halt was triggered partly by a measurement inconsistency. **Branch A is PREMATURE.** The next move is **not** acceptance of negative result and **not** abandonment; it is a **basis-aligned retest** — re-run the experiment (small N, frozen variables otherwise) with the monitor and selector using the same basis. If under basis-aligned conditions the strategy still fails to clear costs and the coint-failure rate stays elevated, the negative result reasserts itself with the artifact ruled out. If under basis-aligned conditions the rate drops materially and edge-vs-cost improves, the original negative result was premature on the universe-fragility leg. Operator-decision row for Branch A flips from `[~] PENDING` to `[ ] NO — re-test under aligned bases instead.`

> **AMBIGUOUS-INSUFFICIENT-PAIRED-DATA** — the data structure doesn't allow a clean pair-on-pair comparison at the same time (entry-discovery and monitor verdicts may not be readily pair-able in the existing logs without re-running the cointegration test on historical data; that re-run is not in scope). The diagnostic does not isolate. **Default routing:** the 6/9 dollar-DECOUPLED cases remain robust economic evidence; the 3/9 TRACKED-THEN-BROKE cases remain ambiguous; the §5 bar's antecedent is met by the convergent stack with one more open caveat recorded. **Branch A holds at "lean accept" with the basis-mismatch question explicitly carried forward** — the operator can choose to (a) accept the negative result with the caveat in the record, or (b) commission a basis-mismatch resolution effort (re-run coint-test under both bases on historical data; or B1 v2 with both bases logged side-by-side). The honest framing: the convergent stack still points one way, but one of the planks under it has a question mark the data can't remove.
