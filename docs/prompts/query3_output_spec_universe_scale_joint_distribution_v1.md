...
- **Virtual-PnL marking fidelity is a VERIFICATION GATE (the x-axis's analogue of the Query-2 z-reconstruction risk).** The whole fork rests on the x-axis (`pnl_at_mean`, `max_in_zone_capture`) being **comparable to the live T1–T8 numbers it is calibrated against.** That holds *only if* the virtual position's mark-to-market PnL is computed on the **exact same basis as live `position_snapshots`** — β-sized legs × live mark, same mark source, same sign convention, same fee/funding treatment in the unrealized figure.
  ...
  **HONEST SCOPE — the gate is no longer purely offline.** Path-1 converts the gate from "bounded offline verification" into "a small read-only live integration that runs concurrent with live trades." It is still **far short of the full harness**: it is a WS subscription to mark-price + a sidecar/logger that reads the `position_snapshots.csv` rows the live bot writes — **no entry-signal generation, no order-placement path, no position state machine, no run-to-N, no cost model.** The validator never writes to bot state, never invokes the trading path, never consumes the live bot's trade-permissioned credentials. **Authorization for this gate is separate from — and a prerequisite to — authorization for the full query-3 build** (the latter remains held; this gate is its prerequisite, not its trigger).
