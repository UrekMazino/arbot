...
- trades_since_experiment_start: 5 (T1 JUP/YGG run_125, T2 LTC/KSM run_126, T3 BNB/LINK run_129, T4 DOGE/AAVE run_130, T5 AVAX/DOT run_131)
- $/σ sign-flip rate: 0/2 = 0% (computable population = T2, T5; both positive. T1, T3, T4 excluded as coint-failures under Rule v1.2)
- aggregate $/σ (pooled): +$0.034/σ (positive; T2 +$0.064, T5 +$0.017)
- coint-failure count (window): 3/5 = 60% (T1 cointegration_lost, T3 cointegration_watch_timeout, T4 cointegration_lost) — down from 3/4=75% after T5 normal exit
- E4 WATCH (not firing): coint-failure 60% sits on the boundary (>60% halt line / 45–60% review band); E4 requires ≥10 closed to evaluate (currently 5). Trending down (75%→60%). Watch continues.
- beta_range_observed: [0.378, 1.495] (T5 β=0.659 inside range); β<1.0: 4/5; non-unity: 5/5; fallback: 0; β-sizing mechanically flawless 5/5 (exact to the cent)
- cumulative PnL (experiment window): -$1.617 (T1: -$0.962, T2: -$0.105, T3: -$0.267, T4: -$0.253, T5: -$0.030)
- edge_clears_costs: 0/2 (T2 gross +$0.146 < costs $0.251; T5 gross +$0.069 < costs $0.100) — BUT T5 MFE +$0.187 > costs: edge exists at peak, exit-capture leaks it (Item 14 exhibit; profit-lock fired but floor $0.120 below cost-clearance)
- action_threshold: 20 closed trades before structural review
- trades_remaining: 15 (to 20 total); ≥6 more $/σ-eligible needed (2 eligible after 5 trades)
...
- audit: docs/audits/exp_beta_aware_sizing_v1_per_run_audit.md (runs 125-131, T1-T5)
...
- run_131: T5 AVAX/DOT, β=0.659, gross=$200 to the cent. 2nd $/σ-ELIGIBLE trade (normal/profit_lock exit, MFE +$0.187>0, Δz=4.00). $/σ=+$0.017 POSITIVE (sign-flip now 0/2). Gross position_pnl +$0.069 (positive — strategy worked) but net -$0.030 (costs ~$0.10). MFE +$0.187 > costs: edge existed at peak, exit-capture leaked it (Item 14). First profit-lock activation in experiment; floor $0.120 below cost-clearance. Coint held (normal exit). Ended via max_session_trades=1.
