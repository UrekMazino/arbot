...
**1.3-RESULT (2026-05-31): the original gate FAILED definitively — 0/15 both stages, 13 trades with >0.5σ single-tick failures.** Artifact: `docs/audits/d1_sanity_check_infeasible_2026-05-31.md`. Root cause: the bot's z is computed from orderbook-mid prices with per-tick OLS β-refit; kline closes with frozen entry-β cannot reproduce it (third converging diagnostic with query-3 §5 and §9.5 — standing meta-finding extracted: the bot's internal basis is not reconstructable from any public historical source).

**1.3-bis — RE-SPECCED GATE (strategist-ratified, v1.2; ONE re-spec only, bind below).**

The original gate asked a stricter question than the pre-test needs. Decomposing what the analysis actually uses: (1) the broken-event triggers are the bot's own logged events — untouched by reconstruction; (2) the outcome variable is a **windowed PnL change**, not a level — and the basis offset ε is structural and stable over short horizons (query-3: consistent ~5.9bps over 28 ticks; §9.5: pair-stable), so it largely cancels in differences; (3) the kline-velocity detector is a **deployable detector in its own right** — a live system can compute exactly it from live klines — so the pre-test tests *that* detector, honestly labeled, rather than pretending to reconstruct the bot's. The detector comparison becomes "bot's logged binary coint detector vs kline-velocity detector" — two real, deployable instruments — which was the design intent anyway.

**The re-specced gate (pre-committed before it runs):** on the in-trade overlap windows, mean abs(**kline-derived position-PnL change** − **logged `upl` change**) over matched 5–15 minute intervals ≤ **$0.03 at $200 notional**, passing on **≥13/15 trades**. This validates the outcome instrument (windowed dollar changes) against ground truth on the only data where ground truth exists.

**THE BIND (strategist, locked):** one re-spec, justified because differencing provably cancels the offset the level-test choked on — a correction of *what is measured*, not of *how hard the test is* (same legitimacy class as the §9.5 classifier correction). **A second re-spec after a second failure would be the iterate-until-passes pattern, and it does not happen.** If the differenced quantities fail this gate, the offset is not stable enough, INFEASIBLE-INSTRUMENT is definitive, the D1 pre-test closes, the kline cache and tooling remain as reusable assets, and the pivot decision returns to the operator with "the premise couldn't be cheaply tested" as its honest status — which itself weighs toward D3 or stop, since a direction whose premise resists even offline testing is a direction you'd be entering blind.

**Implementation notes bound into the re-specced run (strategist's three):**
- **(a) Out-of-trade β:** no logged β exists for non-active-pair-window events. Pre-commit: the hypothetical position's β = OLS on the kline lookback at trigger time (what a deployable system would do), cross-checked against logged β on the in-trade overlap. Query 2 says β is stable; large divergence on the overlap is a flag, not a fudge.
- **(b) Outcome-basis consistency:** the §4.4 confound sub-analysis must compute *both* populations' outcomes on the kline basis — no mixing logged outcomes for in-trade events with kline outcomes for the rest.
- **(c) Timestamp alignment:** the metadata builder's dual-offset timezone handling must be hard-verified against `trade_closes.csv` UTC timestamps before any window extraction. Misaligned timestamps would silently corrupt every window — a worse failure mode than an honest gate fail.

**§6 wording consequence (applies if the re-specced gate passes):** the verdicts claim things about *the detectors actually tested* — the bot's logged broken flag and the kline-velocity detector — not about "the bot's z-velocity," which is unreconstructable. EXISTS or DEAD on those two detectors is what a live experiment would build on anyway, since the live system would deploy one of them.
...
*D1 unified continuation pre-test work item v1.2. Mechanical scaffolding (§0–§5, §8, §9) drafted by code assistant 2026-05-31; verdict definitions (§6) and anti-rationalization lock (§7) authored by strategist and embedded verbatim; v1.1 amendments to §1.3 (velocity-reconstruction validation), §3.1 (single-rule-not-optimized caveat), §3.3 (peak_favorable_excursion field), §6 (Amendment 1: adequacy precondition + VELOCITY-UNDERPOWERED split; Amendment 2: premise-vs-capture split with PEAKS-CLEAR-NET-DOESN'T sub-cause; AMBIGUOUS sub-cause taxonomy), §7 closing calibration. **v1.2 (2026-05-31, post-§1.3 failure):** the original two-stage gate FAILED 0/15 (artifact: `d1_sanity_check_infeasible_2026-05-31.md`; standing meta-finding: internal basis unreconstructable from public sources, third converging diagnostic); strategist ratified ONE re-spec to a differenced-quantity gate (§1.3-bis: kline-PnL-change vs logged-upl-change ≤ $0.03 @ $200 on ≥13/15 trades) with a one-re-spec-only bind — a second failure is definitive INFEASIBLE; tolerance relaxation rejected as gate-shopping; three implementation notes bound (out-of-trade β via kline-OLS cross-checked on overlap; outcome-basis consistency in the confound sub-analysis; timestamp alignment hard-verified before window extraction); §6 verdicts now claim about the detectors actually tested (logged broken flag + kline-velocity detector, both deployable). Read-only / analysis-only. No trades, no bot contact, no frozen-variable touch. Supersedes the prior D1 work item (15-MR-trade sign-flip simulation). Sequenced after Lever-B-DEAD; same anti-rationalization discipline. Hypothesis under test: fast spread divergence at minute-scale on the exp_beta universe predicts continuation large enough to clear costs. Verdict cells: 10 detectors × 4 windows = 40-cell grid; ridge requirement (≥2 adjacent qualifying cells) closes the multiple-comparisons trap; regime split with N guards; confound sub-analysis as permanent meta-finding. AMBIGUOUS expected modal; sub-cause taxonomy keeps it actionable.*
